...
def setup_generator(train_path, test_path, batch_size, dimentions):
    ...
    train_generator = train_datagen.flow_from_directory(
        train_path = r"food-101.tar/images",  # this is the target directory
        target_size=dimentions,
        batch_size=batch_size
        )

    validation_generator = test_datagen.flow_from_directory(
        test_path = r"food-101.tar/images", # this is the target directory
        target_size=dimentions,
        batch_size=batch_size
        )
...
def load_image(img_path, dimentions, rescale=1. / 255):
    ...
    x = ImageDataGenerator(rescale=1. /255) # rescale the same as when trained
